#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║          Elahe Messenger Professional Installer v1.0                   ║
...
print_header() {
    ...
    echo "  ███████╗███████╗██║  ██║██║  ██║███████╗"
    echo "  ╚══════╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝"
    echo -e "        ${CYAN}MESSENGER — Private · Encrypted · Yours${NC}"
    echo -e "${GOLD}================================================================${NC}"
    echo -e "${WHITE}  Installer v1.0 — DNS Control | Auto-SSL | Smart Skip | App v1.0         ${NC}"
    echo -e "${GOLD}================================================================${NC}"
    echo ""
}
...
apply_dns() {
    ...
    if [ -w "/etc/resolv.conf" ]; then
        cp /etc/resolv.conf /etc/resolv.conf.bak.elahe 2>/dev/null || true
    elif [ -n "$S" ]; then
        $S cp /etc/resolv.conf /etc/resolv.conf.bak.elahe 2>/dev/null || true
    fi
...
# Elahe Messenger Caddyfile — Auto-SSL via Let's Encrypt
# Generated by Elahe Messenger Installer v1.0
...
generate_caddyfile_ip_only() {
    ...
    cat > "${TARGET_DIR}/Caddyfile" <<EOF
# Elahe Messenger Caddyfile — IP-only mode
# Generated by Elahe Messenger Installer v1.0
...
setup_ssl_auto_renewal() {
    ...
    local CRON_CMD="0 3 1 * * docker exec \$(docker ps -q -f name=elahe-caddy) caddy reload --config /etc/caddy/Caddyfile 2>/dev/null || true"
    local CRON_COMMENT="# Elahe Messenger — monthly SSL reload"

    # Write to /etc/cron.d for system-wide cron
    local S; S=$(get_sudo)
    local CRON_FILE="/etc/cron.d/elahe-ssl"
    ...
    if [ -n "$S" ]; then
        ...
        ( crontab -l 2>/dev/null | grep -v "Elahe Messenger — monthly SSL"; echo "${CRON_COMMENT}"; echo "${CRON_CMD}" ) | crontab -
...
wait_for_app_health() {
    ...
    for i in $(seq 1 "$ATTEMPTS"); do
        ...
        CONTAINER_STATE=$(docker inspect --format='{{.State.Status}}' elahe-app 2>/dev/null || echo "missing")
        ...
        local APP_STATUS
        APP_STATUS=$(docker inspect --format='{{if .State.Health}}{{.State.Health.Status}}{{else}}no-healthcheck{{end}}' elahe-app 2>/dev/null || echo "unknown")
...
wait_for_caddy_ready() {
    ...
    for i in $(seq 1 "$ATTEMPTS"); do
        local CONTAINER_STATE
        CONTAINER_STATE=$(docker inspect --format='{{.State.Status}}' elahe-caddy 2>/dev/null || echo "missing")
...
launch_services() {
    log_step "Launching Elahe Messenger Services"
    ...
    while [ "$DB_WAIT" -lt 30 ]; do
        if docker inspect --format='{{if .State.Health}}{{.State.Health.Status}}{{end}}' elahe-db 2>/dev/null | grep -q "healthy"; then
    ...
    if wait_for_app_health; then
        ...
        if docker compose up -d caddy; then
            if wait_for_caddy_ready; then
                log_success "All Elahe Messenger services are now operational!"
                DEPLOY_SUCCESS=true
            else
        ...
        if docker compose build --no-cache app && docker compose up -d app && wait_for_app_health; then
            if docker compose up -d caddy && wait_for_caddy_ready; then
                DEPLOY_SUCCESS=true
                log_success "Elahe Messenger services started after recovery build!"
            else
...
print_summary() {
    ...
    if [ "$DEPLOY_SUCCESS" = true ]; then
        ...
        echo -e "${GOLD}╔══════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${GOLD}║          🎉  Elahe Messenger Installation Complete!  🎉               ║${NC}"
        echo -e "${GOLD}╠══════════════════════════════════════════════════════════════════╣${NC}"
        ...
        echo -e "${GOLD}║${NC}"
        echo -e "${GOLD}║${NC}  ${CYAN}Documentation:${NC} https://github.com/ehsanking/ElaheMessenger"
        echo -e "${GOLD}╚══════════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        log_info "Thank you for choosing Elahe Messenger. Stay secure. 👑"
    else
        log_step "Installation Failed ✗"
        echo -e "${RED}╔══════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${RED}║          ⚠  Elahe Messenger Build/Start Failed  ⚠                     ║${NC}"
        echo -e "${RED}╠══════════════════════════════════════════════════════════════════╣${NC}"
        ...
        echo -e "${RED}║${NC}"
        echo -e "${RED}║${NC}  ${CYAN}Support:${NC} https://github.com/ehsanking/ElaheMessenger/issues"
        echo -e "${RED}╚══════════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        exit 1
    fi
