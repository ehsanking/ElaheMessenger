...
capture_reinstall_live_db_env() {
  [ "$INSTALL_MODE" = "reinstall" ] || return 0
  command_exists docker || return 0

  local db_container env_lines key value
  db_container="$(docker ps -a --filter "name=^/elahe-db$" --format '{{.ID}}' 2>/dev/null | head -n1)"
  [ -n "$db_container" ] || return 0

  env_lines="$(docker inspect --format '{{range .Config.Env}}{{println .}}{{end}}' "$db_container" 2>/dev/null || true)"
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      POSTGRES_USER=*|POSTGRES_PASSWORD=*|POSTGRES_DB=*|APP_DB_USER=*|APP_DB_PASSWORD=*)
        key="${line%%=*}"
        value="${line#*=}"
        REINSTALL_LIVE_DB_ENV["$key"]="$value"
        ;;
    esac
  done <<< "$env_lines"
}
...
seed_reinstall_env_from_backup() {
  [ "$INSTALL_MODE" = "reinstall" ] || return 0
  local key required_db_keys
  required_db_keys=(POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DB APP_DB_USER APP_DB_PASSWORD)

  local live_all_required=true
  for key in "${required_db_keys[@]}"; do
    if [ -z "${REINSTALL_LIVE_DB_ENV[$key]-}" ]; then
      live_all_required=false
      break
    fi
  done

  if [ "$live_all_required" = true ]; then
    local live_db_keys
    live_db_keys=(POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DB APP_DB_USER APP_DB_PASSWORD)
    for key in "${live_db_keys[@]}"; do
      env_set_explicit "$key" "${REINSTALL_LIVE_DB_ENV[$key]}"
    done
